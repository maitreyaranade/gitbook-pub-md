...
* [High-Level Synthesis Introduction](ug871.md#high-level-synthesis-introduction)
  * [Lab 1: Creating a High-Level Synthesis Project](ug871.md#lab-1-creating-a-high-level-synthesis-project)
    * [HLS Graphical User Interface (GUI)](ug871.md#hls-graphical-user-interface-gui)
    * [Usual self check operation of test bench](ug871.md#usual-self-check-operation-of-test-bench)
    * [C synthesis report](ug871.md#c-synthesis-report)
    * [RTL co-simulation](ug871.md#rtl-co-simulation)
  * [Lab 2: Using the Tcl Command Interface](ug871.md#lab-2-using-the-tcl-command-interface)
    * [Important points to note](ug871.md#important-points-to-note)
  * [Lab 3: Using Solutions for Design Optimization](ug871.md#lab-3-using-solutions-for-design-optimization)
* [C Validation](ug871.md#c-validation)
  * [Lab 1: C Validation and Debug](ug871.md#lab-1-c-validation-and-debug)
    * [Good practices for writing a testbench](ug871.md#good-practices-for-writing-a-testbench)
  * [Lab 2: C Validation with ANSI C Arbitrary Precision Types](ug871.md#lab-2-c-validation-with-ansi-c-arbitrary-precision-types)
  * [Lab 3: C Validation with C++ Arbitrary Precision Types](ug871.md#lab-3-c-validation-with-c-arbitrary-precision-types)
* [Interface synthesis](ug871.md#interface-synthesis)
  * [Lab 1: Block-Level I/O Protocols](ug871.md#lab-1-block-level-io-protocols)
    * [Important points to be noted](ug871.md#important-points-to-be-noted)
    * [Block level I/O Protocol](ug871.md#block-level-io-protocol)
    * [Types of the block-level interface protocol](ug871.md#types-of-the-block-level-interface-protocol)
  * [Lab 2: Port I/O Protocols](ug871.md#lab-2-port-io-protocols)
    * [Important points to be noted](ug871.md#important-points-to-be-noted-1)
  * [Lab 3: Implementing Arrays as RTL Interfaces](ug871.md#lab-3-implementing-arrays-as-rtl-interfaces)
    * [Important points to be noted](ug871.md#important-points-to-be-noted-2)
  * [Lab 4: Implementing AXI4 Interfaces](ug871.md#lab-4-implementing-axi4-interfaces)
* [Arbitrary Precision Types](ug871.md#arbitrary-precision-types)
  * [Lab 1: Arbitrary Precision](ug871.md#lab-1-arbitrary-precision)
  * [Lab 2: Arbitrary Precision](ug871.md#lab-2-arbitrary-precision)
* [Design Analysis](ug871.md#design-analysis)
  * [Lab 1: Design Optimization](ug871.md#lab-1-design-optimization)
    * [Important points to be noted](ug871.md#important-points-to-be-noted-3)
* [Design Optimization](ug871.md#design-optimization)
  * [Lab 1: Optimizing a Matrix Multiplier](ug871.md#lab-1-optimizing-a-matrix-multiplier)
    * [Aim](ug871.md#aim)
  * [Lab 2: C Code Optimized for I/O Accesses](ug871.md#lab-2-c-code-optimized-for-io-accesses)
* [RTL Verification](ug871.md#rtl-verification)
  * [Lab 1: RTL Verification and the C Test Bench](ug871.md#lab-1-rtl-verification-and-the-c-test-bench)
  * [Lab 2: Viewing Trace Files in Vivado](ug871.md#lab-2-viewing-trace-files-in-vivado)
  * [Lab 3: Viewing Trace Files in ModelSim](ug871.md#lab-3-viewing-trace-files-in-modelsim)
* [Using HLS IP in IP Integrator](ug871.md#using-hls-ip-in-ip-integrator)
  * [Lab 1: Integrate HLS IP with a Xilinx IP Block](ug871.md#lab-1-integrate-hls-ip-with-a-xilinx-ip-block)
* [Using HLS IP in a Zynq AP SoC Design](ug871.md#using-hls-ip-in-a-zynq-ap-soc-design)
  * [Lab 1: Implement Vivado HLS IP on a Zynq Device](ug871.md#lab-1-implement-vivado-hls-ip-on-a-zynq-device)
  * [Lab 2: Streaming Data Between the Zynq CPU and HLS Accelerator Blocks](ug871.md#lab-2-streaming-data-between-the-zynq-cpu-and-hls-accelerator-blocks)
...
::: highlight Reference for this sections: Vivado Design Suite Tutorial High-Level Synthesis UG871 :::
...
::: center ![Vivado HLS Graphical User Interface](../HLS/images/panes.png){#panes width="80%"} :::
...
3. Run **"vivado\_hls --f run\_hls.tcl"** in the the Vivado HLS Command Prompt (within required directory).
...
* The actual and expected results are compared. If the comparison fails, the value of variable err\_cnt is set to a non-zero value.
...
::: highlight IMPORTANT: When working with arbitrary precision types you can use the Vivado HLS debug environment only with C++ or SystemC. When using arbitrary precision types with ANSI C,the debug environment cannot be used. With ANSI C, you must instead use printf or fprintf statements for debugging. :::
...
::: highlight Arbitrary precision types are a powerful means to create high-performance, bit accurate hardware designs. :::
...
Behavior of the signals for block-level I/O protocol ap\_ctrl\_hs: Insert table here
...
* If ap\_start is held high, the next transaction will start on the next clock cycle.
...
* _Cosim only supports the following 'ap\_ctrl\_none' designs:_
  1. Combinational designs
  2. Pipelined design with task interval of 1
  3. Designs with array streaming or hls\_stream ports

#### Types of the block-level interface protocol

There are four types of the block-level interface protocol:

1. ap\_ctrl\_none: No block-level I/O control protocol. When the interface protocol ap\_ctrl\_none is used, no block-level I/O protocols are added to the design. The only ports are those for the clock, reset and the data ports.
2. ap\_ctrl\_hs: The block-level I/O control handshake protocol. This protocol is associated with the function return value (this is true even if the function has no return value specified in the code).
3. ap\_ctrl\_chain: The block-level I/O protocol for control chaining. This I/O protocol is primarily used for chaining pipelined blocks together. In addition to the ap\_ctrl\_hs protocol but with an additional input signal, ap\_continue, which must be high when ap\_done is asserted for the next transaction to proceed. This allows downstream blocks to apply back-pressure on the system and halt further processing when they are unable to continue accepting new data.
4. s\_axilite: Can be applied in addition to ap\_ctrl\_hs or ap\_ctrl\_chain to implement the block-level I/O protocol as an AXI Slave Lite interface in place of separate discrete I/O ports.
...
* Pass-by-value arguments can be implemented with the following I/O protocols:
  1. ap\_none: No I/O protocol. This is the default for inputs.
  2. ap\_stable: No I/O protocol.
  3. ap\_ack: Implemented with an associated output acknowledge port.
  4. ap\_vld: Implemented with an associated input valid port.
  5. ap\_hs: Implemented with both input valid and output acknowledge ports.
* Pass-by-reference arguments that can be implemented with the following I/O protocols:
  1. ap\_none: No I/O protocol. This is the default for inputs.
  2. ap\_stable: No I/O protocol.
  3. ap\_ack: Implemented with an associated input acknowledge port.
  4. ap\_vld: Implemented with an associated output valid port. This is the default for outputs.
  5. ap\_ovld: Implemented with an associated output valid port (no valid port for the input part of any inout ports).
  6. ap\_hs: Implemented with both input valid port and output acknowledge ports
  7. ap\_fifo: A FIFO interface with associated output write and input FIFO full ports.
  8. ap\_bus: A Vivado HLS bus interface protocol.
...
::: highlight When AXI4-Lite interface is added, the IP packaging process creates software driver files to enable an external block, typically a CPU, to control this block (start, stop , set port values, review the interrupt status). :::
...
::: highlight Vivado High-Level Synthesis (HLS) provides a number of bit accurate or arbitrary precision data-types, allowing you to model variables using any (arbitrary) width. :::

### Lab 1: Arbitrary Precision

This lab synthesizes the same function used in Lab 1 using arbitrary precision fixed-types highlighting how the same design can be converted to the Vivado HLS ap\_fixed types, retaining the required accuracy but creating a more optimal hardware implementation.

1. Creating a New Project
2. Review Test Bench and Run C Simulation
3. Synthesize the Design and Review Results

::: highlight High-Level Synthesis can synthesize floating-point types directly into hardware, provided the operations are standard arithmetic operations (+, -, \*, %). :::
...
::: highlight Changing data types from standard C types to arbitrary precision types, make sure to reduce the size of the data types. This results in smaller operators, reduced area, and fewer clock cycles to complete. :::
